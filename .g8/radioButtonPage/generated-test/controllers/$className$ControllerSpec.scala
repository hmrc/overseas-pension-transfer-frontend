...
class $className$ControllerSpec extends AnyFreeSpec with SpecBase with MockitoSugar {
  ...
  "$className$ Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = userAnswersQtNumber.set($className$Page, $className$.values.head).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill($className$.values.head), NormalMode)(fakeDisplayRequest(request), messages(application)).toString
      }
    }
