...
import forms.transferDetails.assetsMiniJourney.unquotedShares.UnquotedSharesCompanyNameFormProvider
import models.Mode
import pages.transferDetails.assetsMiniJourney.unquotedShares.UnquotedSharesCompanyNamePage
import play.api.i18n.{I18nSupport, MessagesApi}
import play.api.mvc.{Action, AnyContent, MessagesControllerComponents}
import repositories.SessionRepository
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController
import views.html.transferDetails.assetsMiniJourney.unquotedShares.UnquotedSharesCompanyNameView
  ...
  def onSubmit(mode: Mode, index: Int): Action[AnyContent] = (identify andThen getData andThen requireData andThen displayData).async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(UnquotedSharesCompanyNamePage(index), value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(UnquotedSharesCompanyNamePage(index).nextPage(mode, updatedAnswers))
      )
  }
}
