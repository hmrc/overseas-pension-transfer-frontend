...
class PropertyDescriptionController @Inject() (
  ...
  def onPageLoad(mode: Mode, index: Int): Action[AnyContent] = (identify andThen getData andThen requireData andThen displayData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(PropertyDescriptionPage(index)) match {
        case None        => form
  ...
  def onSubmit(mode: Mode, index: Int, fromFinalCYA: Boolean): Action[AnyContent] = (identify andThen getData andThen requireData andThen displayData).async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(PropertyDescriptionPage(index), value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(PropertyDescriptionPage(index).nextPage(mode, updatedAnswers, fromFinalCYA))
      )
  }
}
