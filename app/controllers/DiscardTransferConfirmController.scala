  ...
  def onSubmit(): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(DiscardTransferConfirmPage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(DiscardTransferConfirmPage.nextPage(NormalMode, updatedAnswers))
      )
  }
}
