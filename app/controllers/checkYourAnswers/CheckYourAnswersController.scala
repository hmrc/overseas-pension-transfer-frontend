...
class CheckYourAnswersController @Inject() (
  ...
  def onPageLoad(): Action[AnyContent] = (identify andThen getData andThen requireData andThen displayData) {
    implicit request =>
      val memberDetailsSummaryList        = SummaryListViewModel(MemberDetailsSummary.rows(request.userAnswers))
      val transferDetailsSummaryList      = SummaryListViewModel(TransferDetailsSummary.rows(request.userAnswers))
      val qropsDetailsSummaryList         = SummaryListViewModel(QROPSDetailsSummary.rows(request.userAnswers))
      val schemeManagerDetailsSummaryList = SummaryListViewModel(SchemeManagerDetailsSummary.rows(request.userAnswers))
