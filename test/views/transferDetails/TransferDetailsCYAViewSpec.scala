...
class TransferDetailsCYAViewSpec extends ViewBaseSpec {

  private val view = applicationBuilder().injector().instanceOf[TransferDetailsCYAView]

  private val summaryList = SummaryList()

  "TransferDetailsCYAView" - {

    "show correct title" in {
      doc(view(summaryList).body).getElementsByTag("title").eachText().get(0) mustBe
        s"${messages("checkYourAnswers.transferDetails.title")} - ${messages("service.name")} - GOV.UK"
    }

    behave like pageWithH1(view(summaryList), "checkYourAnswers.transferDetails.heading")
