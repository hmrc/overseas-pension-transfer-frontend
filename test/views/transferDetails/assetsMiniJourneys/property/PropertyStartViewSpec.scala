...
class PropertyStartViewSpec extends ViewBaseSpec {

  private val view = applicationBuilder().injector().instanceOf[PropertyStartView]
  ...
  "PropertyStartView" - {
    ...
    behave like pageWithH1(view(nextPage), "propertyStart.heading")
    ...
    "display informational text" in {
      ...
      paragraphs.first().text() mustBe messages("propertyStart.text")
    }
