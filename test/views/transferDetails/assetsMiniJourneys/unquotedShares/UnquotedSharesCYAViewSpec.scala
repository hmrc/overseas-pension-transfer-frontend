...
class UnquotedSharesCYAViewSpec extends ViewBaseSpec {

  private val view = applicationBuilder().injector().instanceOf[UnquotedSharesCYAView]

  private val summaryList = SummaryList()
  private val testIndex   = 0

  "UnquotedSharesCYAView" - {
    ...
    behave like pageWithH1(view(summaryList, NormalMode, testIndex), "checkYourAnswers.heading")

    behave like pageWithSubmitButton(view(summaryList, NormalMode, testIndex), "site.continue")

    "display summary list" in {
      val summaryLists = doc(view(summaryList, NormalMode, testIndex).body)
