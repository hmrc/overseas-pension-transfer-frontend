...
class MoreQuotedSharesDeclarationViewSpec extends ViewBaseSpec {

  private val view         = applicationBuilder().injector().instanceOf[MoreQuotedSharesDeclarationView]
  private val formProvider = applicationBuilder().injector().instanceOf[MoreQuotedSharesDeclarationFormProvider]
  ...
  "MoreQuotedSharesDeclarationView" - {

    "show correct title" in {
      doc(view(formProvider(), testAssets, NormalMode).body)
        .getElementsByTag("title").eachText().get(0) mustBe
        s"${messages("moreQuotedSharesDeclaration.title")} - ${messages("service.name")} - GOV.UK"
    }
    ...
    "display dynamic heading with asset count" in {
      ...
      heading.text() mustBe messages("additionalAsset.common.title", testAssets.length, messages("quotedSharesAmendContinue.text.plural"))
    }
    ...
    behave like pageWithErrors(
      view(formProvider().withError(FormError("value", "moreQuotedSharesDeclaration.error.required")), testAssets, NormalMode),
      "value",
      "moreQuotedSharesDeclaration.error.required"
    )
  }
}
