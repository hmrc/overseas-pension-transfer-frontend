...
class QuotedSharesStartViewSpec extends ViewBaseSpec {

  private val view = applicationBuilder().injector().instanceOf[QuotedSharesStartView]

  private val nextPage = "/next-page-url"

  "QuotedSharesStartView" - {
    ...
    behave like pageWithH1(view(nextPage), "quotedSharesStart.heading")
    ...
    "display informational text" in {
      ...
      paragraphs.first().text() mustBe messages("quotedSharesStart.text")
    }
