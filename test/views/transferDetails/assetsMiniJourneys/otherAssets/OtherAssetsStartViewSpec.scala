...
class OtherAssetsStartViewSpec extends ViewBaseSpec {

  private val view = applicationBuilder().injector().instanceOf[OtherAssetsStartView]

  private val nextPage = "/next-page-url"

  "OtherAssetsStartView" - {
    ...
    behave like pageWithH1(view(nextPage), "otherAssetsStart.heading")
    ...
    "display informational text" in {
      ...
      paragraphs.first().text() mustBe messages("otherAssetsStart.text")
    }
