...
class OtherAssetsConfirmRemovalViewSpec extends ViewBaseSpec {

  private val view         = applicationBuilder().injector().instanceOf[OtherAssetsConfirmRemovalView]
  private val formProvider = applicationBuilder().injector().instanceOf[OtherAssetsConfirmRemovalFormProvider]

  private val testIndex = 0

  "OtherAssetsConfirmRemovalView" - {
    ...
    behave like pageWithH1(view(formProvider(), testIndex), "otherAssetsConfirmRemoval.heading")
    ...
    behave like pageWithErrors(
      view(formProvider().withError(FormError("value", "otherAssetsConfirmRemoval.error.required")), testIndex),
      "value",
      "otherAssetsConfirmRemoval.error.required"
    )
  }
}
