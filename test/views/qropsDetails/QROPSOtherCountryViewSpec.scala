...
class QROPSOtherCountryViewSpec extends ViewBaseSpec {

  private val view         = applicationBuilder().injector().instanceOf[QROPSOtherCountryView]
  private val formProvider = applicationBuilder().injector().instanceOf[QROPSOtherCountryFormProvider]

  "QROPSOtherCountryView" - {

    "show correct title" in {
      doc(view(formProvider(), NormalMode).body).getElementsByTag("title").eachText().get(0) mustBe
        s"${messages("qropsOtherCountry.title")} - ${messages("service.name")} - GOV.UK"
    }
