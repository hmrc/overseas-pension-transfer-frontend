...
class MemberDateOfBirthViewSpec extends ViewBaseSpec {

  private val view         = applicationBuilder().injector().instanceOf[MemberDateOfBirthView]
  private val formProvider = applicationBuilder().injector().instanceOf[MemberDateOfBirthFormProvider]

  "MemberDateOfBirthView" - {

    "show correct title" in {
      doc(view(formProvider(), NormalMode).body).getElementsByTag("title").eachText().get(0) mustBe
        s"${messages("memberDateOfBirth.title")} - ${messages("service.name")} - GOV.UK"
    }
    ...
    "display correct heading with member name" in {
      ...
      heading.text() mustBe messages("memberDateOfBirth.heading", testMemberName.fullName)
    }
    ...
    behave like pageWithErrors(
      view(formProvider().withError(FormError("value", "memberDateOfBirth.error.required.all")), NormalMode),
      "value",
      "memberDateOfBirth.error.required.all"
    )
  }
}
