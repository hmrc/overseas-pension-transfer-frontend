...
class MemberDateOfLeavingUKViewSpec extends ViewBaseSpec {

  private val view         = applicationBuilder().injector().instanceOf[MemberDateOfLeavingUKView]
  private val formProvider = applicationBuilder().injector().instanceOf[MemberDateOfLeavingUKFormProvider]

  "MemberDateOfLeavingUKView" - {

    "show correct title" in {
      doc(view(formProvider(), NormalMode).body).getElementsByTag("title").eachText().get(0) mustBe
        s"${messages("memberDateOfLeavingUK.title")} - ${messages("service.name")} - GOV.UK"
    }
    ...
    "display correct heading with member name" in {
      ...
      heading.text() mustBe messages("memberDateOfLeavingUK.heading", testMemberName.fullName)
    }
    ...
    behave like pageWithErrors(
      view(formProvider().withError(FormError("value", "memberDateOfLeavingUK.error.required.all")), NormalMode),
      "value",
      "memberDateOfLeavingUK.error.required.all"
    )
  }
}
