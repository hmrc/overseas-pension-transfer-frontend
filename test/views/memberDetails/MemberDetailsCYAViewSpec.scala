...
class MemberDetailsCYAViewSpec extends ViewBaseSpec {

  private val view = applicationBuilder().injector().instanceOf[MemberDetailsCYAView]

  private val summaryList = SummaryList()

  "MemberDetailsCYAView" - {

    "show correct title" in {
      doc(view(summaryList).body).getElementsByTag("title").eachText().get(0) mustBe
        s"${messages("checkYourAnswers.memberDetails.title")} - ${messages("service.name")} - GOV.UK"
    }

    behave like pageWithH1(view(summaryList), "checkYourAnswers.memberDetails.heading")
