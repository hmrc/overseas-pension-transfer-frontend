...
class SchemeManagersNameViewSpec extends ViewBaseSpec {

  private val view         = applicationBuilder().injector().instanceOf[SchemeManagersNameView]
  private val formProvider = applicationBuilder().injector().instanceOf[SchemeManagersNameFormProvider]

  "SchemeManagersNameView" - {

    "show correct title" in {
      doc(view(formProvider(), NormalMode).body).getElementsByTag("title").eachText().get(0) mustBe
        s"${messages("schemeManagersName.title")} - ${messages("service.name")} - GOV.UK"
    }

    behave like pageWithH1(view(formProvider(), NormalMode), "schemeManagersName.heading")

    behave like pageWithMultipleInputFields(
      view(formProvider(), NormalMode),
      ("schemeManagersFirstName", "common.nameInput.firstName"),
      ("schemeManagersLastName", "common.nameInput.lastName")
    )
