...
class SchemeManagerDetailsCYAViewSpec extends ViewBaseSpec {

  private val view = applicationBuilder().injector().instanceOf[SchemeManagerDetailsCYAView]

  private val summaryList = SummaryList()

  "SchemeManagerDetailsCYAView" - {

    "show correct title" in {
      doc(view(summaryList).body).getElementsByTag("title").eachText().get(0) mustBe
        s"${messages("checkYourAnswers.QROPSSchemeManagerDetails.title")} - ${messages("service.name")} - GOV.UK"
    }

    behave like pageWithH1(view(summaryList), "checkYourAnswers.QROPSSchemeManagerDetails.heading")
