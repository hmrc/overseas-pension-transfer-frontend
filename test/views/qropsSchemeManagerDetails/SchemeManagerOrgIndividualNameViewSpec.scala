...
class SchemeManagerOrgIndividualNameViewSpec extends ViewBaseSpec {

  private val view         = applicationBuilder().injector().instanceOf[SchemeManagerOrgIndividualNameView]
  private val formProvider = applicationBuilder().injector().instanceOf[SchemeManagerOrgIndividualNameFormProvider]

  "SchemeManagerOrgIndividualNameView" - {

    "show correct title" in {
      doc(view(formProvider(), NormalMode).body).getElementsByTag("title").eachText().get(0) mustBe
        s"${messages("orgIndividualName.title")} - ${messages("service.name")} - GOV.UK"
    }

    behave like pageWithH1(view(formProvider(), NormalMode), "orgIndividualName.heading")

    behave like pageWithMultipleInputFields(
      view(formProvider(), NormalMode),
      ("orgIndFirstName", "common.nameInput.firstName"),
      ("orgIndLastName", "common.nameInput.lastName")
    )
