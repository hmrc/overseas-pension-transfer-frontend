  ...
  "AssetThresholdHandler" - {
    ...
    "getAssetCount" - {
      ...
      "must return 0 if no assets present" in {
        ...
        AssetThresholdHandler.getAssetCount(userAnswers, TypeOfAsset.Property) mustEqual 0
      }

      "must return correct count for given asset type" in {
        val userAnswers = userAnswersWithAssets(assetsCount = 2)
        AssetThresholdHandler.getAssetCount(userAnswers, TypeOfAsset.Property) mustEqual 2
      }

      "must return 0 for unknown asset type (no data)" in {
        val userAnswers = emptyUserAnswers
        AssetThresholdHandler.getAssetCount(userAnswers, TypeOfAsset.QuotedShares) mustEqual 0
      }
    }
    ...
    "handle" - {
      ...
      "must set flag false if asset count < threshold" in {
        ...
        val updated     = AssetThresholdHandler.handle(userAnswers, TypeOfAsset.Property)
      ...
      "must set flag to userSelection if asset count == threshold" in {
        ...
        val updatedTrue = AssetThresholdHandler.handle(userAnswers, TypeOfAsset.Property, Some(true))
        (updatedTrue.data \ "transferDetails" \ "moreProp").as[Boolean] mustBe true

        val updatedFalse = AssetThresholdHandler.handle(userAnswers, TypeOfAsset.Property, Some(false))
      ...
      "must set flag false even if no assets for known asset type" in {
        val userAnswers = emptyUserAnswers
        val updated     = AssetThresholdHandler.handle(userAnswers, TypeOfAsset.QuotedShares)
      ...
      "must return unchanged UserAnswers for unknown asset type" in {
        val userAnswers = emptyUserAnswers
        val updated     = AssetThresholdHandler.handle(userAnswers, TypeOfAsset.Other)
