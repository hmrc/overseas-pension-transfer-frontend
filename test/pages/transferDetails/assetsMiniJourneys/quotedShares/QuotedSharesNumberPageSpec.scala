...
class QuotedSharesNumberPageSpec extends AnyFreeSpec with Matchers with SpecBase {
  private val index = 0
  ...
  ".nextPage" - {
    ...
    "in Check Mode" - {
      ...
      "must go to Check Answers" in {
        ...
        QuotedSharesNumberPage(index).nextPage(CheckMode, emptyAnswers) mustEqual AssetsMiniJourneysRoutes.QuotedSharesCYAController.onPageLoad(index)
      }
    }

    "in FinalCheckMode" - {
      "must go to Final Check Answers page" in {
        QuotedSharesNumberPage(index).nextPage(FinalCheckMode, emptyAnswers) mustEqual
