  ...
  ".nextPage" - {
    ...
    "in NormalMode" - {
      ...
      "must go to the cya page if no-continue selected and no more assets" in {
        val userAnswers = emptyUserAnswers.set(QuotedSharesAmendContinueAssetPage, false).success.value
        val nextIndex   = 2
        QuotedSharesAmendContinueAssetPage.nextPageWith(
          NormalMode,
          userAnswers,
      ...
      "must go to the next asset page if no-continue selected and more assets" in {
        val userAnswers = emptyUserAnswers.set(QuotedSharesAmendContinueAssetPage, false)
        val sessionData =
        ...
        val result = QuotedSharesAmendContinueAssetPage.nextPageWith(
          NormalMode,
