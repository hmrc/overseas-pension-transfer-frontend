...
class MoreUnquotedSharesDeclarationPageSpec extends AnyFreeSpec with Matchers with SpecBase {

  ".nextPage" - {

    "in NormalMode" - {
      "must go to cya if no more assets" in {
        MoreUnquotedSharesDeclarationPage.nextPageWith(NormalMode, emptyUserAnswers, sessionDataMemberName) mustEqual
          routes.TransferDetailsCYAController.onPageLoad()
      }

      "must go to the next asset page if more assets" in {
        val userAnswers = emptyUserAnswers.set(MoreUnquotedSharesDeclarationPage, false)
        val sessionData =
    ...
    "in CheckMode" - {
      "must go to cya if no more assets" in {
        MoreUnquotedSharesDeclarationPage.nextPageWith(CheckMode, emptyUserAnswers, sessionDataMemberName) mustEqual
          routes.TransferDetailsCYAController.onPageLoad()
      }
    ...
    "in FinalCheckMode" - {
      "must go to cya if no more assets" in {
        MoreUnquotedSharesDeclarationPage.nextPage(FinalCheckMode, emptyUserAnswers) mustEqual
    ...
    "in AmendCheckMode" - {
      "must go to cya if no more assets" in {
        MoreUnquotedSharesDeclarationPage.nextPage(AmendCheckMode, emptyUserAnswers) mustEqual
