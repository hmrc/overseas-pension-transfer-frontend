...
class UnquotedSharesConfirmRemovalControllerSpec extends AnyFreeSpec with SpecBase with MockitoSugar {
  ...
  val unquotedSharesConfirmRemovalRoute = routes.UnquotedSharesConfirmRemovalController.onPageLoad().url
  ...
  "UnquotedSharesConfirmRemoval Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual routes.AdditionalUnquotedShareController.onPageLoad(NormalMode).url
      }
    }
