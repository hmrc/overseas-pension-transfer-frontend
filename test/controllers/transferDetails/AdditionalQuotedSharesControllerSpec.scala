...
class AdditionalQuotedSharesControllerSpec extends AnyFreeSpec with SpecBase with MockitoSugar {

  private val formProvider = new AdditionalQuotedSharesFormProvider()
  private val form         = formProvider()

  private lazy val additionalQuotedShareRoute = routes.AdditionalQuotedSharesController.onPageLoad().url
  ...
  "AdditionalQuotedShare Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[AdditionalQuotedSharesView]
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual routes.QuotedSharesCompanyNameController.onPageLoad(NormalMode, 0).url
      }
    }
