  ...
  "MorePropertyDeclaration Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val rows = PropertyAmendContinueSummary.rows(userAnswersWithProperty(5))

        status(result) mustEqual OK
        contentAsString(result) mustEqual
          view(form, rows, NormalMode)(fakeDisplayRequest(request, userAnswersWithProperty(5)), messages(application)).toString
      }
    }
