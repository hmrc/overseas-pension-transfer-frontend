...
class PropertyCYAControllerSpec extends AnyFreeSpec with SpecBase with MockitoSugar {
  ...
  "PropertyCYA Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      val application = applicationBuilder(userAnswers = Some(userAnswersWithProperty(5))).build()
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual
          view(SummaryList(rows = list), 0)(fakeDisplayRequest(request, userAnswersWithProperty(5)), msgs).toString
      }
    }
    ...
    "must redirect to PropertyAmendContinueController when property count is below threshold" in {
      val application = applicationBuilder(userAnswers = Some(userAnswersWithProperty(4))).build()
