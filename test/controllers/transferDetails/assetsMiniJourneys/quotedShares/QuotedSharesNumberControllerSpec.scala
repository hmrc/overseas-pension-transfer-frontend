...
class QuotedSharesNumberControllerSpec extends AnyFreeSpec with SpecBase with MockitoSugar {
  ...
  "NumberOfQuotedShares Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(QuotedSharesNumberPage(index), validAnswer).success.value
    ...
    "must redirect to the next page when valid data is submitted in AmendCheckMode" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, AssetsMiniJourneysRoutes.QuotedSharesNumberController.onPageLoad(AmendCheckMode, index).url)
            .withFormUrlEncodedBody(("value", validAnswer.toString))

        val result = route(application, request).value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual QuotedSharesNumberPage(index).nextPage(AmendCheckMode, emptyUserAnswers).url
      }
    }
