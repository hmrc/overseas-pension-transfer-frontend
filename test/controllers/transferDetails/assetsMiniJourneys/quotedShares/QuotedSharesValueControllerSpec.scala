...
class QuotedSharesValueControllerSpec extends AnyFreeSpec with SpecBase with MockitoSugar {
  ...
  "QuotedSharesValue Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(QuotedSharesValuePage(index), validAnswer).success.value
    ...
    "must redirect to the next page when valid data is submitted in AmendCheckMode" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, AssetsMiniJourneysRoutes.QuotedSharesValueController.onPageLoad(AmendCheckMode, index).url)
            .withFormUrlEncodedBody(("value", validAnswer.toString))
