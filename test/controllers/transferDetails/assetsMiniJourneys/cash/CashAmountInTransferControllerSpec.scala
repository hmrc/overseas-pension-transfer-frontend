...
class CashAmountInTransferControllerSpec extends AnyFreeSpec with SpecBase with MockitoSugar {
  ...
  "CashAmountInTransfer Controller" - {
    ...
    "must redirect to the next page when valid data is submitted in AmendCheckMode" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, AssetsMiniJourneysRoutes.CashAmountInTransferController.onPageLoad(AmendCheckMode).url)
            .withFormUrlEncodedBody(("cashInTransfer", validAnswer.toString))
