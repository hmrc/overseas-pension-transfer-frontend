...
import forms.transferDetails.assetsMiniJourney.quotedShares.QuotedSharesNumberFormProvider
import models.NormalMode
import org.mockito.ArgumentMatchers.any
import org.mockito.Mockito.when
import org.scalatest.freespec.AnyFreeSpec
import org.scalatestplus.mockito.MockitoSugar
import pages.transferDetails.assetsMiniJourney.quotedShares.QuotedSharesNumberPage
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers._
import repositories.SessionRepository
import views.html.transferDetails.assetsMiniJourney.quotedShares.QuotedSharesNumberView

import scala.concurrent.Future

class QuotedSharesNumberControllerSpec extends AnyFreeSpec with SpecBase with MockitoSugar {
  ...
  "NumberOfQuotedShares Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = userAnswersQtNumber.set(QuotedSharesNumberPage(index), validAnswer).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual QuotedSharesNumberPage(index).nextPage(NormalMode, emptyUserAnswers).url
      }
    }
