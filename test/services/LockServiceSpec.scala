...
class LockServiceSpec extends AnyFreeSpec with Matchers with SpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  private val service = new LockService(mockMongoLockRepository, mockAuditService)
  ...
  override def beforeEach(): Unit =
    reset(mockMongoLockRepository, mockAuditService)
  ...
  "LockService" - {
    ...
    "must acquire lock successfully and trigger StartJourney audit" in {
      ...
      when(mockMongoLockRepository.takeLock(eqTo(transferId.value), eqTo(owner)))
      ...
      verify(mockMongoLockRepository).takeLock(eqTo(transferId.value), eqTo(owner))
    ...
    "must acquire and release lock using releaseLock" in {
      when(mockMongoLockRepository.takeLock(eqTo(transferId.value), eqTo(owner)))
        .thenReturn(Future.successful(None))
      when(mockMongoLockRepository.releaseLock(eqTo(transferId.value), eqTo(owner)))
      ...
      await(service.releaseLock(transferId.value, owner))
      verify(mockMongoLockRepository).takeLock(eqTo(transferId.value), eqTo(owner))
      verify(mockMongoLockRepository).releaseLock(eqTo(transferId.value), eqTo(owner))
    }

    "must return true when lock is currently locked by the owner (isLocked)" in {
      when(mockMongoLockRepository.isLocked(eqTo("lock1"), eqTo(owner)))
        .thenReturn(Future.successful(true))

      val result = await(service.isLocked("lock1", owner))
      result mustBe true

      verify(mockMongoLockRepository).isLocked(eqTo("lock1"), eqTo(owner))
    }

    "must return false when lock is not held by the owner (isLocked)" in {
      when(mockMongoLockRepository.isLocked(eqTo("lock2"), eqTo(owner)))
        .thenReturn(Future.successful(false))

      val result = await(service.isLocked("lock2", owner))
      result mustBe false

      verify(mockMongoLockRepository).isLocked(eqTo("lock2"), eqTo(owner))
    }
