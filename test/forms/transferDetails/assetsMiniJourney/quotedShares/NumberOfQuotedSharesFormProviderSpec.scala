...
class NumberOfQuotedSharesFormProviderSpec extends StringFieldBehaviours with Regex {

  val form = new NumberOfQuotedSharesFormProvider()()
  ...
  ".value" - {
    ...
    behave like mandatoryField(
      form,
      fieldName,
      requiredError = FormError(fieldName, "numberOfQuotedShares.error.required")
    )
    ...
    "fail to bind non-numeric value" in {
      ...
      result.errors must contain only FormError(fieldName, "numberOfQuotedShares.error.invalid")
    }

    "fail to bind negative number" in {
      val result = form.bind(Map(fieldName -> "-50"))
      result.errors must contain only FormError(fieldName, "numberOfQuotedShares.error.negative")
    }
